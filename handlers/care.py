...
@router.message(User_Status.choosing_care, F.text.lower() == "подушки")
...
@router.message(User_Status.choosing_care, F.text.lower() == "одеяла")
...
@router.message(User_Status.choosing_care, F.text.lower() == "наматрацники")
async def select_odeiala(message: Message, state: FSMContext):
    album_builder = MediaGroupBuilder()
    album_builder.add(
        type="photo",
