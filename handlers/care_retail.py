...
@router.message(User_Status.selected_care_by_retail, F.text.lower() == "подушки")
async def select_podushki(message: Message, state: FSMContext):
    ...
    await state.set_state(User_Status.end_care_by_retail)


@router.message(User_Status.selected_care_by_retail, F.text.lower() == "одеяла")
...
@router.message(User_Status.selected_care_by_retail, F.text.lower() == "наматрацники")
